const newsArticles = [
 {
      id: 1,
      title: "Dar es Salaam Regional Commissioner Visits Lake Cement on Workers' Day",
      title_sw: "Mkuu wa Mkoa wa Dar es Salaam Azuru Lake Cement kwenye Siku ya Wafanyakazi",
      excerpt: "The Dar es Salaam Regional Commissioner, Hon. Chalamila, has announced upcoming inspections of workplaces, including factories, to assess employee relations and working conditions. Lake Cement welcomed this initiative as part of our commitment to maintaining excellent labor standards.",
      excerpt_sw: "Mkuu wa Mkoa wa Dar es Salaam, Mhe. Chalamila, ametangaza ukaguzi wa sehemu za kazi, ikiwemo viwanda, kuchunguza uhusiano wa wafanyakazi na mazingira ya kazi. Lake Cement imekaribisha mpango huu kama sehemu ya dhamira yetu ya kudumisha viwango bora vya kazi.",
      fullContent: {
        intro: "In a significant move to strengthen labor practices across the region, Dar es Salaam Regional Commissioner Hon. Albert Chalamila made an official visit to Lake Cement's state-of-the-art manufacturing facility on Workers' Day. During his tour, the Commissioner announced comprehensive workplace inspections aimed at enhancing employee welfare and ensuring regulatory compliance across the industrial sector.",
        intro_sw: "Katika hatua muhimu ya kuimarisha mazoea ya kazi katika mkoa, Mkuu wa Mkoa wa Dar es Salaam Mhe. Albert Chalamila alifanya ziara rasmi katika kiwanda cha kisasa cha Lake Cement siku ya Wafanyakazi. Wakati wa ziara yake, Mkuu wa Mkoa alitangaza ukaguzi kamili wa sehemu za kazi unaolenga kuboresha ustawi wa wafanyakazi na kuhakikisha uzingatiaji wa kanuni katika sekta ya viwanda.",
        paragraphs: [
          "The Regional Commissioner's visit came as part of the government's broader initiative to improve working conditions and labor relations in Tanzania's growing industrial sector. Hon. Chalamila was accompanied by senior regional officials and representatives from the Ministry of Labor, who conducted a thorough tour of Lake Cement's production facilities, employee amenities, and safety protocols.",
          "During the facility inspection, the Regional Commissioner paid particular attention to health and safety measures, employee welfare facilities, and the implementation of labor regulations. The visit highlighted the Tanzanian government's commitment to ensuring that industrial growth does not come at the expense of worker welfare and rights.",
          "Lake Cement, as a leading manufacturer in the region, has long maintained rigorous standards for workplace safety and employee relations. The company's exemplary practices in these areas were acknowledged during the visit, with Hon. Chalamila noting that such standards should be emulated across the industrial sector."
        ],
        paragraphs_sw: [
          "Ziara ya Mkuu wa Mkoa ilikuja kama sehemu ya mpango mpana wa serikali wa kuboresha mazingira ya kazi na mahusiano ya kazi katika sekta ya viwanda inayokua Tanzania. Mhe. Chalamila aliambatana na maafisa wakuu wa mkoa na wawakilishi kutoka Wizara ya Kazi, ambao walifanya ziara kamili ya vifaa vya uzalishaji vya Lake Cement, vitendea kazi vya wafanyakazi, na itifaki za usalama.",
          "Wakati wa ukaguzi wa kiwanda, Mkuu wa Mkoa alitilia mkazo hatua za afya na usalama, vitendea kazi vya ustawi wa wafanyakazi, na utekelezaji wa kanuni za kazi. Ziara hiyo ilisisitizia dhamira ya serikali ya Tanzania ya kuhakikisha kuwa ukuaji wa viwanda hauji kwa gharama ya ustawi na haki za wafanyakazi.",
          "Lake Cement, kama mtengenezaji mkuu katika mkoa, imekuwa ikidumisha viwango kali vya usalama mahali pa kazi na uhusiano wa wafanyakazi. Mazoea ya mfano ya kampuni katika maeneo haya yalitambuliwa wakati wa ziara, Mhe. Chalamila akitaja kuwa viwango kama hivyo vinapaswa kuigwa katika sekta nzima ya viwanda."
        ],
        quote: {
          text: "Companies like Lake Cement that prioritize worker welfare while maintaining high production standards demonstrate that business excellence and employee wellbeing can go hand in hand. We want to ensure all industries in Dar es Salaam region follow such exemplary practices.",
          text_sw: "Kampuni kama Lake Cement zinazoweka ustawi wa wafanyakazi kwenye kipaumbele wakati wa kudumisha viwango vya juu vya uzalishaji zinaonyesha kuwa ubora wa biashara na ustawi wa wafanyakazi vinaweza kwenda sambamba. Tunataka kuhakikisha viwanda vyote katika mkoa wa Dar es Salaam vinafuata mazoea bora kama haya.",
          author: "Hon. Albert Chalamila, Dar es Salaam Regional Commissioner",
          author_sw: "Mhe. Albert Chalamila, Mkuu wa Mkoa wa Dar es Salaam"
        },
        additionalContent: [
          "The announced inspections will focus on several key areas including workplace safety protocols, adherence to minimum wage requirements, proper implementation of employee benefits, and compliance with environmental regulations. These inspections are scheduled to commence in the coming weeks and will cover factories across various industrial sectors in the Dar es Salaam region.",
          "Lake Cement's management expressed their full support for the initiative, viewing it as an opportunity to showcase their commitment to best practices in the industry. The company has consistently invested in employee development, safety training, and welfare programs as part of its corporate philosophy.",
          "As one of Tanzania's premier cement manufacturers, Lake Cement has established itself as a leader not only in product quality but also in corporate responsibility. The company's approach to employee relations includes comprehensive health insurance, professional development opportunities, and engagement programs that foster a positive workplace culture."
        ],
        additionalContent_sw: [
          "Ukaguzi uliotangazwa utazingatia maeneo kadhaa muhimu ikiwa ni pamoja na itifaki za usalama mahali pa kazi, uzingatiaji wa mahitaji ya mshahara wa chini, utekelezaji sahihi wa manufaa ya wafanyakazi, na uzingatiaji wa kanuni za mazingira. Ukaguzi huu umepangwa kuanza katika wiki zijazo na utajumuisha viwanda katika sekta mbalimbali za viwanda katika mkoa wa Dar es Salaam.",
          "Uongozi wa Lake Cement ulielezea msaada wao kamili kwa mpango huu, wakiuona kama fursa ya kuonyesha dhamira yao ya kufuata mazoea bora katika tasnia. Kampuni imekuwa ikiwekeza kwa uthabiti katika maendeleo ya wafanyakazi, mafunzo ya usalama, na mipango ya ustawi kama sehemu ya falsafa yake ya kampuni.",
          "Kama mojawapo ya watengenezaji wakuu wa saruji Tanzania, Lake Cement imejiimarisha kama kiongozi si tu katika ubora wa bidhaa lakini pia katika wajibu wa kampuni. Mtazamo wa kampuni kuhusu mahusiano ya wafanyakazi unajumuisha bima kamili ya afya, fursa za maendeleo ya kitaaluma, na mipango ya ushirikishwaji inayojenga utamaduni chanya wa mahali pa kazi."
        ],
        conclusion: "The Regional Commissioner's Workers' Day visit to Lake Cement underscores the important partnership between government and private sector in ensuring industrial development that benefits all stakeholders. Lake Cement remains committed to maintaining the highest standards in both production excellence and employee welfare as we continue to contribute to Tanzania's infrastructure development and economic growth.",
        conclusion_sw: "Ziara ya Mkuu wa Mkoa ya Siku ya Wafanyakazi kwenye Lake Cement inasisitiza ushirikiano muhimu kati ya serikali na sekta binafsi katika kuhakikisha maendeleo ya viwanda yanayofaidisha wadau wote. Lake Cement inaendelea kujitolea kudumisha viwango vya juu zaidi katika ubora wa uzalishaji na ustawi wa wafanyakazi tunapoendelea kuchangia maendeleo ya miundombinu ya Tanzania na ukuaji wa uchumi."
      },
      date: "2024-06-15",
      author: "Lake Cement Communications Team",
      author_sw: "Timu ya Mawasiliano ya Lake Cement",
      mainImage: "/images/news/RC-Chalamila.png",
      gallery: [
        "/images/news/RC-Chalamila.png",
      ],
      category: "company",
      featured: true,
      tag: "Recent",
      externalLink: "https://www.mwananchi.co.tz/mw/habari/kitaifa/ukaguzi-wa-haki-za-wafanyakazi-viwandani-waja--4659252",
      relatedArticles: [2, 6, 4]
    },    {
      id: 2,
      title: "RC Chalamila Conducts Tour of Kigamboni District",
      title_sw: "RC Chalamila Afanya Ziara ya Wilaya ya Kigamboni",
      excerpt: "Dar es Salaam Regional Commissioner Albert Chalamila conducted an extensive tour of Kigamboni District to inspect various development projects and engage with community members. During his visit, he emphasized the government's commitment to infrastructure development and public service improvement.",
      excerpt_sw: "Mkuu wa Mkoa wa Dar es Salaam Albert Chalamila amefanya ziara kubwa katika Wilaya ya Kigamboni kukagua miradi mbalimbali ya maendeleo na kuwasiliana na wanajamii. Wakati wa ziara yake, alisisitizia kujitolea kwa serikali katika maendeleo ya miundombinu na uboreshaji wa huduma za umma.",
      fullContent: {
        intro: "Dar es Salaam Regional Commissioner Hon. Albert Chalamila recently completed a comprehensive inspection tour of Kigamboni District, focusing on critical infrastructure projects that are transforming the rapidly developing area. The tour included visit to Nyati Cement Factory as great contributor to the area's development agenda.",
        intro_sw: "Mkuu wa Mkoa wa Dar es Salaam Mhe. Albert Chalamila hivi karibuni amekamilisha ziara ya ukaguzi wa kina ya Wilaya ya Kigamboni, akizingatia miradi muhimu ya miundombinu inayobadilisha eneo linaloendelea kwa kasi. Ziara hiyo ilijumuisha kutembelea Kiwanda cha Saruji cha Nyati kama mchangiaji mkubwa katika ajenda ya maendeleo ya eneo hilo.",
        paragraphs: [
          "The Regional Commissioner's visit to Kigamboni highlights the district's strategic importance in Dar es Salaam's urban expansion plans. With its coastal location and growing population, Kigamboni has become a focal point for infrastructure investment, with numerous roads, bridges, schools, and healthcare facilities under construction or renovation.",
          "During his tour, Hon. Chalamila inspected progress on the Kigamboni Coastal Road project, a vital transportation artery that will significantly improve connectivity between Kigamboni and the city center. The project utilizes Lake Cement's premium products, which have been selected for their exceptional durability in coastal environments where exposure to saltwater and high humidity presents unique challenges.",
          "The Regional Commissioner also visited several public buildings under construction, including the new Kigamboni District Hospital and two secondary schools. These projects form part of the government's commitment to improving public services in the rapidly growing district, with proper infrastructure serving as the foundation for community development."
        ],
        paragraphs_sw: [
          "Ziara ya Mkuu wa Mkoa huko Kigamboni inasisitiza umuhimu wa kimkakati wa wilaya hiyo katika mipango ya upanuzi wa mjini Dar es Salaam. Kwa sababu ya eneo lake la pwani na idadi ya watu inayoongezeka, Kigamboni imekuwa kitovu cha uwekezaji wa miundombinu, na barabara nyingi, madaraja, shule, na vituo vya afya vikiwa vinajengwa au kufanyiwa ukarabati.",
          "Wakati wa ziara yake, Mhe. Chalamila alikagua maendeleo ya mradi wa Barabara ya Pwani ya Kigamboni, mishipa muhimu ya usafiri ambayo itaboresha kwa kiasi kikubwa muunganisho kati ya Kigamboni na katikati ya jiji. Mradi huo unatumia bidhaa bora za Lake Cement, ambazo zimechaguliwa kwa sababu ya uimara wao wa kipekee katika mazingira ya pwani ambapo kuwa karibu na maji ya chumvi na unyevu wa juu kunaletea changamoto za kipekee.",
          "Mkuu wa Mkoa pia alitembelea majengo kadhaa ya umma yanayojengwa, ikiwa ni pamoja na Hospitali mpya ya Wilaya ya Kigamboni na shule mbili za sekondari. Miradi hii ni sehemu ya dhamira ya serikali ya kuboresha huduma za umma katika wilaya inayokua kwa kasi, kwa miundombinu bora ikiwa kama msingi wa maendeleo ya jamii."
        ],
        quote: {
          text: "The quality of construction materials used in these projects is paramount to ensuring long-term sustainability. I'm pleased to see locally manufactured products like Lake Cement being utilized, which not only ensures quality but also supports our industrial growth strategy.",
          text_sw: "Ubora wa vifaa vya ujenzi vinavyotumika katika miradi hii ni muhimu sana kuhakikisha uendelevu wa muda mrefu. Nina furaha kuona bidhaa zinazozalishwa hapa nchini kama vile Lake Cement zikitumika, ambazo si tu zinahakikisha ubora lakini pia zinasaidia mkakati wetu wa ukuaji wa viwanda.",
          author: "Hon. Albert Chalamila, Dar es Salaam Regional Commissioner",
          author_sw: "Mhe. Albert Chalamila, Mkuu wa Mkoa wa Dar es Salaam"
        },
        additionalContent: [
          "Lake Cement's involvement in these development projects extends beyond simply supplying materials. Our technical team has provided expertise and guidance to contractors working on the Kigamboni projects, ensuring optimal application of our cement products in the challenging coastal environment.",
          "Community engagement was another significant aspect of the Regional Commissioner's tour, with Hon. Chalamila holding meetings with local leaders and residents to discuss development priorities. Infrastructure development consistently ranked as the top priority for local residents, who emphasized the need for durable construction that can withstand the coastal climate.",
          "As part of our commitment to the communities where our products are used, Lake Cement has implemented a technical support program for government projects in Kigamboni. This includes training sessions for local contractors on best practices for cement application in coastal conditions and quality assurance protocols."
        ],
        additionalContent_sw: [
          "Ushiriki wa Lake Cement katika miradi hii ya maendeleo unaenda zaidi ya kusambaza vifaa tu. Timu yetu ya kiufundi imetoa utaalam na mwongozo kwa makandarasi wanaofanya kazi kwenye miradi ya Kigamboni, kuhakikisha matumizi bora ya bidhaa zetu za saruji katika mazingira magumu ya pwani.",
          "Ushirikishwaji wa jamii ulikuwa kipengele kingine muhimu cha ziara ya Mkuu wa Mkoa, ambapo Mhe. Chalamila alifanya mikutano na viongozi wa mitaa na wakazi kujadili vipaumbele vya maendeleo. Maendeleo ya miundombinu yalikuwa yameorodheshwa kama kipaumbele cha juu kwa wakazi wa eneo hilo, ambao walisisitizia umuhimu wa ujenzi wenye uimara ambao unaweza kustahimili hali ya hewa ya pwani.",
          "Kama sehemu ya dhamira yetu kwa jamii ambapo bidhaa zetu zinatumika, Lake Cement imetekeleza programu ya msaada wa kiufundi kwa miradi ya serikali huko Kigamboni. Hii inajumuisha vipindi vya mafunzo kwa makandarasi wa mitaa juu ya mazoea bora ya matumizi ya saruji katika hali za pwani na itifaki za uhakikisho wa ubora."
        ],
        conclusion: "The Regional Commissioner's tour of Kigamboni District underscores the critical role that quality construction materials play in Tanzania's infrastructure development. Lake Cement remains committed to supporting these efforts through our premium products and technical expertise, contributing to the durability and longevity of essential public infrastructure across the country.",
        conclusion_sw: "Ziara ya Mkuu wa Mkoa katika Wilaya ya Kigamboni inasisitiza jukumu muhimu ambalo vifaa bora vya ujenzi vinachukua katika maendeleo ya miundombinu ya Tanzania. Lake Cement inaendelea kujitolea kusaidia juhudi hizi kupitia bidhaa zetu bora na utaalam wa kiufundi, kuchangia katika uimara na maisha marefu ya miundombinu muhimu ya umma nchini kote."
      },
      date: "2024-05-15",
      author: "Lake Cement Communications Team",
      author_sw: "Timu ya Mawasiliano ya Lake Cement",
      mainImage: "/images/news/chalamila.jpg",
      gallery: [
        "/images/news/chalamila.jpg"
    
      ],
      category: "company",
      featured: true,
      externalLink: "https://dsm.go.tz/new/rc-chalamila-afanya-ziara-kigamboni",
      relatedArticles: [1, 3, 6]
    },    {
      id: 3,
      title: "Nyati Cement Donates Cement Bags for School Construction",
      title_sw: "Nyati Cement Yachangia Mifuko ya Saruji kwa Ujenzi wa Shule",
      excerpt: "Nyati Cement Donates 400 bags to the Bagamoyo District Commissioner for the construction of School. Showing our commitment to CSR and support for education infrastructure development in local communities.",
      excerpt_sw: "Nyati Cement imechangia mifuko 400 kwa Mkuu wa Wilaya ya Bagamoyo kwa ujenzi wa Shule. Ikionyesha dhamira yetu ya CSR na msaada kwa maendeleo ya miundombinu ya elimu katika jamii za mitaa.",
      fullContent: {
        intro: "In a significant demonstration of our commitment to education and community development, Nyati Cement has donated 400 bags of premium cement to support the construction of a new secondary school in Bagamoyo District. This donation will directly contribute to improving educational infrastructure in a community where access to quality learning facilities has been limited.",
        intro_sw: "Katika onyesho kubwa la dhamira yetu kwa elimu na maendeleo ya jamii, Nyati Cement imechangia mifuko 400 ya saruji bora kusaidia ujenzi wa shule mpya ya sekondari katika Wilaya ya Bagamoyo. Mchango huu utachangia moja kwa moja kuboresha miundombinu ya elimu katika jamii ambapo upatikanaji wa vifaa vya kujifunzia vya ubora umekuwa mdogo.",
        paragraphs: [
          "Education infrastructure remains a critical challenge across Tanzania, with many communities lacking adequate facilities to support quality learning environments. The new school in Bagamoyo will address this gap by providing modern classrooms built to high standards using Nyati Cement's durable products, ensuring the building will serve generations of students.",
          "This initiative aligns with Nyati Cement's broader corporate social responsibility strategy, which focuses on sustainable community development through targeted investments in education, health, and infrastructure. By supporting local education infrastructure, the company aims to contribute to Tanzania's human capital development goals while strengthening relationships with the communities where it operates."
        ],
        paragraphs_sw: [
          "Miundombinu ya elimu inabaki kuwa changamoto muhimu katika Tanzania nzima, ambapo jamii nyingi zinakosa vifaa vya kutosha kusaidia mazingira bora ya kujifunzia. Shule mpya huko Bagamoyo itashughulikia pengo hili kwa kutoa madarasa ya kisasa yaliyojengwa kwa viwango vya juu kwa kutumia bidhaa za kudumu za Nyati Cement, kuhakikisha jengo litahudumia vizazi kwa vizazi.",
          "Mpango huu unaafikiana na mkakati mpana wa uwajibikaji wa shirika la Nyati Cement, ambao unazingatia maendeleo endelevu ya jamii kupitia uwekezaji uliolengwa katika elimu, afya, na miundombinu. Kwa kusaidia miundombinu ya elimu ya mitaa, kampuni inalenga kuchangia malengo ya maendeleo ya rasilimali watu ya Tanzania huku ikiimarisha uhusiano na jamii ambapo inafanya kazi."
        ],
        quote: {
          text: "Access to quality education facilities is fundamental to improving learning outcomes for our children. This is genourous investment and commitment to our community's future.",
          text_sw: "Upatikanaji wa vifaa bora vya elimu ni muhimu kwa kuboresha matokeo ya kujifunza kwa watoto wetu. Huu ni uwekezaji mkubwa na dhamira kwa mustakabali wa jamii yetu.",
          author: "IPN Pathak, Lake Cement Stategic Business Head",
          author_sw: "IPN Pathak, Mkuu wa Biashara wa Kimkakati wa Lake Cement"
        },
        additionalContent: [
          "Beyond the material donation, Nyati Cement has also provided technical support for the construction project, with company engineers offering guidance on optimal cement application techniques to ensure building durability. This knowledge transfer component enhances the long-term impact of the donation by building local capacity for quality construction.",
          "The School project represents a collaborative effort between private sector, government, and local communities. While Nyati Cement has provided essential building materials, community members are contributing labor, and the district government is overseeing implementation to ensure adherence to national education infrastructure standards.",
          "Education remains one of the primary focus areas for Nyati Cement's corporate social responsibility initiatives"
        ],
        additionalContent_sw: [
          "Zaidi ya utoaji wa vifaa, Nyati Cement pia imetoa msaada wa kiufundi kwa mradi wa ujenzi, ambapo wahandisi wa kampuni wanatoa mwongozo wa mbinu bora za matumizi ya saruji kuhakikisha jengo linakuwa imara. Kipengele hiki cha uhamisho wa maarifa kinaongeza athari za muda mrefu za mchango kwa kujenga uwezo wa mitaa kwa ujenzi wa ubora.",
          "Mradi wa Shule unawakilisha juhudi za pamoja kati ya sekta binafsi, serikali, na jamii za mitaa. Wakati Nyati Cement imetoa vifaa muhimu vya ujenzi, wanajamii wanachangia nguvu kazi, na serikali ya wilaya inasimamia utekelezaji kuhakikisha uzingatiaji wa viwango vya kitaifa vya miundombinu ya elimu.",
          "Elimu inabaki kuwa moja ya maeneo ya msingi ya kuzingatia kwa mpango wa uwajibikaji wa kijamii wa kampuni ya Nyati Cement"
        ],
        conclusion: "The cement donation to Bagamoyo District exemplifies Nyati Cement's philosophy of investing in tangible, sustainable development initiatives that address real community needs. By supporting education infrastructure, we're not just building classrooms – we're helping build Tanzania's future through improved access to quality education facilities.",
        conclusion_sw: "Mchango wa saruji kwa Wilaya ya Bagamoyo ni mfano wa falsafa ya Nyati Cement ya kuwekeza katika miradi halisi na endelevu ya maendeleo inayoshughulikia mahitaji halisi ya jamii. Kwa kusaidia miundombinu ya elimu, hatujeng tu madarasa - tunasaidia kujenga mustakabali wa Tanzania kupitia upatikanaji bora wa vifaa vya elimu ya ubora."
      },
      date: "2016-11-25",
      author: "Corporate Affairs Department",
      author_sw: "Idara ya Masuala ya Shirika",
      mainImage: "/images/news/5.jpg",
      gallery: [
        "/images/news/5.jpg", 
        "/images/news/6.jpg",
        "/images/news/2.jpg",
        "/images/news/11.jpg"
      ],
      category: "csr",
      featured: true,
      relatedArticles: [5, 2, 4]
    },        {
            id: 4,
            title: "Lake Cement Plants Trees in the Annual Ceremony",
            title_sw: "Lake Cement Inapanda Miti katika Sherehe ya Kila Mwaka",
            excerpt: "Lake Cement demonstrated its commitment to environmental sustainability through planting tress in the annual ceremony, which also featured employee recognition awards and team-building activities at the company's factory at Kimbiji.",
            excerpt_sw: "Lake Cement imeonyesha dhamira yake ya uhifadhi wa mazingira kupitia upandaji wa miti katika sherehe ya kila mwaka, ambayo pia ilijumuisha tuzo za kutambua wafanyakazi na shughuli za kujenga timu katika kiwanda cha kampuni huko Kimbiji.",
            fullContent: {
              intro: "Lake Cement recently held its annual ceremony where employees and management planted trees around the company's manufacturing facility in Kigamboni. The event, which has become a cherished tradition at the company, combined environmental conservation efforts with employee recognition and team-building activities.",
              intro_sw: "Lake Cement hivi karibuni ilifanya sherehe yake ya kila mwaka ambapo wafanyakazi na wasimamizi walipanda miti karibu na kiwanda cha uzalishaji cha kampuni huko Kigamboni. Tukio hili, ambalo limekuwa desturi inayothaminiwa katika kampuni, lilichanganya juhudi za uhifadhi wa mazingira na utambuzi wa wafanyakazi na shughuli za kujenga timu.",
              paragraphs: [
                "The tree planting initiative is part of Lake Cement's broader environmental sustainability program that aims to offset carbon emissions and improve air quality in the surrounding communities. This year's ceremony saw participation from all departments, with executive leadership working alongside production staff to plant a diverse range of native tree species selected for their adaptability to local conditions and ecological benefits.",
                "Beyond the environmental focus, the event served as a platform for the company's annual employee recognition awards. Outstanding achievements in safety, innovation, production excellence, and environmental stewardship were celebrated, with several team members receiving commendations for their exceptional contributions to the company's success over the past year.",
                "In his address during the ceremony, Lake Cement's Managing Director emphasized that environmental responsibility and operational excellence go hand in hand, noting that the company's commitment to sustainability has improved both its environmental performance and business outcomes through enhanced efficiency and resource conservation."
              ],
              paragraphs_sw: [
                "Mpango wa upandaji miti ni sehemu ya programu pana ya uendelevu wa mazingira ya Lake Cement inayolenga kupunguza uzalishaji wa kaboni na kuboresha ubora wa hewa katika jamii zinazozunguka. Sherehe ya mwaka huu ilishuhudia ushiriki kutoka idara zote, ambapo uongozi wa juu ulifanya kazi pamoja na wafanyakazi wa uzalishaji kupanda aina mbalimbali za miti ya asili iliyochaguliwa kwa sababu ya uwezo wake wa kuambatana na hali za eneo hilo na faida za kiikolojia.",
                "Zaidi ya kuzingatia mazingira, tukio hili lilitumika kama jukwaa la tuzo za kila mwaka za kutambua wafanyakazi. Mafanikio bora katika usalama, ubunifu, uzalishaji bora, na usimamizi wa mazingira yalisherehekewa, ambapo wanatimu kadhaa walipokea pongezi kwa michango yao ya kipekee kwa mafanikio ya kampuni katika mwaka uliopita.",
                "Katika hotuba yake wakati wa sherehe, Mkurugenzi Mtendaji wa Lake Cement alisisitizia kuwa wajibu wa mazingira na ubora wa utendaji vinaenda sambamba, akieleza kuwa dhamira ya kampuni ya uendelevu imeboresha utendaji wake wa mazingira na matokeo ya biashara kupitia ufanisi ulioboreshwa na uhifadhi wa rasilimali."
              ],
              quote: {
                text: "Our annual tree planting ceremony represents our philosophy that sustainability isn't just about reducing negative impacts—it's about creating positive ones. Each tree we plant today symbolizes our shared commitment to building a sustainable future for our company, our communities, and Tanzania as a whole.",
                text_sw: "Sherehe yetu ya kila mwaka ya upandaji wa miti inawakilisha falsafa yetu kwamba uendelevu sio tu kuhusu kupunguza athari hasi—ni kuhusu kuunda athari chanya. Kila mti tunaopanda leo unaashiria dhamira yetu ya pamoja ya kujenga mustakabali endelevu kwa kampuni yetu, jamii zetu, na Tanzania kwa ujumla.",
                author: "Kush Patel,  Director, Lake Cement",
                author_sw: "Kush Patel, Mkurugenzi, Lake Cement"
              },
              additionalContent: [
                "The event included various team-building activities designed to strengthen collaboration across departments. Employees participated in sports and collaborative challenges that reinforced the company's core values while building camaraderie among the workforce.",
                "Lake Cement's environmental initiatives have evolved significantly over the years, with the tree planting program expanding from an initial 100 trees in its first year to over 500 this year. The company maintains a dedicated nursery on its premises, where seedlings are grown throughout the year in preparation for the annual planting ceremony.",
                "Community representatives were also invited to participate in this year's event, highlighting Lake Cement's efforts to engage local stakeholders in its sustainability journey. Local schools received donations of seedlings to establish their own tree planting programs, extending the environmental benefits beyond the company's immediate grounds."
              ],
              additionalContent_sw: [
                "Tukio hili lilijumuisha shughuli mbalimbali za kujenga timu zilizoundwa kuimarisha ushirikiano kati ya idara. Wafanyakazi walishiriki katika michezo na changamoto za ushirikiano zilizoimarisha thamani za msingi za kampuni huku zikiimarisha udugu miongoni mwa nguvu kazi.",
                "Mipango ya mazingira ya Lake Cement imekua kwa kiasi kikubwa katika miaka, na programu ya upandaji miti ikipanuka kutoka miti 100 ya awali katika mwaka wake wa kwanza hadi zaidi ya 500 mwaka huu. Kampuni inadumisha kitalu cha miti katika eneo lake, ambapo miche inakuzwa kwa mwaka mzima kwa matayarisho ya sherehe ya upandaji ya kila mwaka.",
                "Wawakilishi wa jamii pia walikaribishwa kushiriki katika tukio la mwaka huu, ikionyesha juhudi za Lake Cement za kuwahusisha wadau wa eneo hilo katika safari yake ya uendelevu. Shule za mitaa zilipokea michango ya miche ili kuanzisha programu zao za upandaji miti, na kupanua faida za mazingira zaidi ya maeneo ya karibu ya kampuni."
              ],
              conclusion: "As Lake Cement continues to balance industrial production with environmental stewardship, initiatives like the annual tree planting ceremony demonstrate how corporate values can be translated into tangible actions. By combining environmental conservation with employee recognition, the company reinforces its commitment to sustainability while building a corporate culture that values both people and planet.",
              conclusion_sw: "Lake Cement inapoendelea kuwianisha uzalishaji wa viwanda na usimamizi wa mazingira, mipango kama sherehe ya kila mwaka ya upandaji miti inaonyesha jinsi thamani za kampuni zinaweza kutafsiriwa kuwa vitendo halisi. Kwa kuchanganya uhifadhi wa mazingira na utambuzi wa wafanyakazi, kampuni inaimarisha dhamira yake ya uendelevu huku ikijengea utamaduni wa kampuni unaothamini watu na sayari."
            },            date: "2022-07-26",
            author: "",
            author_sw: "",
            mainImage: "/images/news/env.webp",
            gallery: [
              "/images/news/env.webp"
            ],
            category: "company",
            relatedArticles: [6, 1, 3]
          },{
        id: 5,
        title: "Lake Cement Leads Blood Donation Drive to Save Lives",
        title_sw: "Lake Cement Yaongoza Kampeni ya Kuchangia Damu Kuokoa Maisha",
        excerpt: "Lake Cement has demonstrated corporate social responsibility through an impactful blood donation drive at our factory. The initiative aims to address the critical shortage of blood supplies in Tanzania's healthcare system and highlights our dedication to community health and wellbeing.",
        excerpt_sw: "Lake Cement imeonyesha uwajibikaji wa shirika kupitia kampeni yenye athari ya kuchangia damu katika kiwanda chetu. Mpango huu unalenga kushughulikia upungufu mkubwa wa vifaa vya damu katika mfumo wa afya wa Tanzania na unasisitiza kujitolea kwetu kwa afya na ustawi wa jamii.",
        fullContent: {
          intro: "Lake Cement has taken a leading role in addressing Tanzania's critical blood shortage by hosting a successful blood donation drive at its Kimbiji factory in Kigamboni, Dar es Salaam. The initiative, conducted in partnership with the National Blood Transfusion Service's Eastern Zone, forms part of the company's commitment to supporting vital healthcare needs in the communities where it operates.",
          intro_sw: "Lake Cement imechukua jukumu la uongozi katika kushughulikia upungufu mkubwa wa damu Tanzania kwa kuandaa kampeni ya kuchangia damu iliyofanikiwa katika kiwanda chake cha Kimbiji kilichopo Kigamboni, Dar es Salaam. Mpango huu, uliofanywa kwa ushirikiano na Huduma ya Kitaifa ya Kuhamisha Damu ya Kanda ya Mashariki, ni sehemu ya dhamira ya kampuni ya kusaidia mahitaji muhimu ya afya katika jamii ambapo inafanya kazi.",
          paragraphs: [
            "During the event, Fatuma Mjungu, Mobilization Officer for the National Safe Blood Program in the Eastern Zone, emphasized the urgent need for safe blood donations due to the high number of patients requiring transfusions to save their lives. She revealed that in the previous year, they had collected 160,000 units of blood, falling short of their target of 300,000 units. With strategic initiatives like the Lake Cement blood drive, she expressed confidence in reaching their collection goal this year through continued public awareness and corporate partnerships.",
            "Ms. Mjungu commended Lake Cement for their dedication to blood donation efforts and encouraged other manufacturing companies to follow their example by hosting similar drives at their facilities. This type of corporate engagement plays a crucial role in helping the National Blood Program meet its collection targets and ensure adequate blood supplies for hospitals across the region.",
            "Ahmed Nyang'anyi, Human Resources Officer at Lake Cement, highlighted the company's recognition of blood's critical importance in saving lives, particularly for mothers during childbirth and various other patient groups. He committed that Lake Cement would continue organizing annual blood donation drives, acknowledging that the need for blood far exceeds the current collection rates in Tanzania."
          ],
          paragraphs_sw: [
            "Wakati wa tukio hili, Fatuma Mjungu, Afisa wa Uhamasishaji wa Programu ya Kitaifa ya Damu Salama katika Kanda ya Mashariki, alisisitizia haja ya haraka ya michango ya damu salama kutokana na idadi kubwa ya wagonjwa wanaohitaji kuhamishiwa damu kuokoa maisha yao. Alifunua kwamba mwaka uliopita, walikuwa wamekusanya vitengo 160,000 vya damu, wakikosa lengo lao la vitengo 300,000. Kwa mipango ya kimkakati kama ya kampeni ya kuchangia damu ya Lake Cement, alielezea imani ya kufikia lengo lao la ukusanyaji mwaka huu kupitia uhamasishaji wa umma unaoendelea na ushirikiano wa mashirika.",
            "Bi. Mjungu alisifu Lake Cement kwa kujitolea kwao kwa juhudi za kuchangia damu na kuhimiza kampuni nyingine za uzalishaji kufuata mfano wao kwa kuandaa kampeni kama hizi katika viwanda vyao. Aina hii ya ushiriki wa shirika ina jukumu muhimu katika kusaidia Programu ya Kitaifa ya Damu kufikia malengo yake ya ukusanyaji na kuhakikisha upatikanaji wa kutosha wa damu kwa hospitali katika kanda nzima.",
            "Ahmed Nyang'anyi, Afisa wa Rasilimali Watu katika Lake Cement, alisisitizia utambuzi wa kampuni juu ya umuhimu mkubwa wa damu katika kuokoa maisha, hasa kwa akina mama wakati wa kujifungua na makundi mbalimbali ya wagonjwa. Alitoa ahadi kuwa Lake Cement itaendelea kuandaa kampeni za kila mwaka za kuchangia damu, akitambua kuwa mahitaji ya damu yanazidi sana viwango vya sasa vya ukusanyaji nchini Tanzania."
          ],
          quote: {
            text: "There is no factory that produces blood, so everyone has a responsibility to donate blood to save the lives of those in need. Even those who donate may themselves need blood someday, as no one knows when they might require a transfusion.",
            text_sw: "Hakuna kiwanda kinachozalisha damu, kwa hiyo kila mtu ana wajibu wa kuchangia damu kuokoa maisha ya wale walio katika mahitaji. Hata wale wanaochangia wanaweza wenyewe kuhitaji damu siku moja, kwani hakuna anayejua wakati wanaweza kuhitaji kuhamishiwa damu.",
            author: "Happy Mboma, Health, Environment and Workplace Manager, Lake Cement",
            author_sw: "Happy Mboma, Meneja wa Afya, Mazingira na Mahali pa Kazi, Lake Cement"
          },
          additionalContent: [
            "The Vice President of Lake Cement, Afroz Ansari, personally participated in the blood donation drive, demonstrating leadership through example and encouraging employees to contribute to this life-saving initiative. His participation underscored the company's senior management commitment to corporate social responsibility and community health.",
            "The blood donation drive saw enthusiastic participation from Lake Cement employees, reflecting the company's success in fostering a culture of community service and social responsibility among its workforce. By creating awareness about blood donation within the organization, Lake Cement is helping to build a sustainable culture of regular blood donation.",
            "This initiative reflects Lake Cement's holistic approach to corporate social responsibility, which extends beyond environmental considerations and economic development to include direct interventions in community health and wellbeing. By addressing the blood shortage challenge, the company is leveraging its resources and community presence to support a critical national health need."
          ],
          additionalContent_sw: [
            "Makamu wa Rais wa Lake Cement, Afroz Ansari, alishiriki binafsi katika kampeni ya kuchangia damu, akionyesha uongozi kupitia mfano na kuhimiza wafanyakazi kuchangia katika mpango huu wa kuokoa maisha. Ushiriki wake ulisisitiza dhamira ya uongozi wa juu wa kampuni kwa uwajibikaji wa shirika na afya ya jamii.",
            "Kampeni ya kuchangia damu ilishuhudia ushiriki wa shauku kutoka kwa wafanyakazi wa Lake Cement, ikiakisi mafanikio ya kampuni katika kukuza utamaduni wa huduma za jamii na uwajibikaji wa kijamii miongoni mwa nguvu kazi yake. Kwa kuunda ufahamu kuhusu kuchangia damu ndani ya shirika, Lake Cement inasaidia kujenga utamaduni endelevu wa kuchangia damu mara kwa mara.",
            "Mpango huu unaakisi mtazamo kamili wa Lake Cement juu ya uwajibikaji wa shirika, ambao unaenda zaidi ya kuzingatia mazingira na maendeleo ya kiuchumi hadi kujumuisha mipango ya moja kwa moja katika afya na ustawi wa jamii. Kwa kushughulikia changamoto ya upungufu wa damu, kampuni inatumia rasilimali zake na uwepo wake katika jamii kusaidia mahitaji muhimu ya afya ya kitaifa."
          ],
          conclusion: "Through initiatives like this blood donation drive, Lake Cement continues to demonstrate that corporate responsibility extends beyond manufacturing excellence to include active participation in addressing critical community and national challenges. The company plans to make this a regular event, contributing to Tanzania's healthcare system while raising awareness about the ongoing need for blood donations across the country.",
          conclusion_sw: "Kupitia mipango kama hii ya kampeni ya kuchangia damu, Lake Cement inaendelea kuonyesha kuwa wajibu wa shirika unaenda zaidi ya ubora wa uzalishaji kujumuisha ushiriki wa kufanya kazi katika kushughulikia changamoto muhimu za jamii na kitaifa. Kampuni inapanga kufanya tukio hili kuwa la mara kwa mara, kuchangia mfumo wa afya wa Tanzania huku ikiongeza ufahamu kuhusu mahitaji yanayoendelea ya michango ya damu nchi nzima."
        },
        date: "2017-05-17",
        author: "CSR Department",
        author_sw: "Idara ya CSR",
        mainImage: "/images/news/damu4.webp",
        gallery: [  
        "/images/news/damu4.webp",
        "/images/news/damu2.webp",
        "/images/news/damu1.webp",
        "/images/news/damu3.webp"
        ],
        category: "csr",
        externalLink: "https://www.michuzi.co.tz/2017/05/lake-cement-yachangia-damu-katika.html",
        relatedArticles: [3, 7, 1]
    },    {
      id: 6,
      title: "Prime Minister Majaliwa Assures Lake Cement of Government Support",
      title_sw: "Waziri Mkuu Majaliwa Ahakikishia Lake Cement Msaada wa Serikali",
      excerpt: "During his visit to Lake Cement's state-of-the-art factory, Prime Minister Kassim Majaliwa praised our production quality and affirmed the government's commitment to supporting local manufacturers. The PM emphasized the importance of private sector investment in strengthening Tanzania's industrial capacity.",
      excerpt_sw: "Wakati wa ziara yake kwenye kiwanda cha kisasa cha Lake Cement, Waziri Mkuu Kassim Majaliwa alisifu ubora wa uzalishaji wetu na kuthibitisha kujitolea kwa serikali kusaidia wazalishaji wa ndani. Waziri Mkuu alisisitizia umuhimu wa uwekezaji wa sekta binafsi katika kuimarisha uwezo wa viwanda wa Tanzania.",
      fullContent: {
        intro: "In a significant endorsement of local manufacturing excellence, Prime Minister Kassim Majaliwa paid an official visit to Lake Cement's production facility, where he commended the company's contribution to Tanzania's industrial development and assured ongoing government support for local manufacturers. The high-profile visit underscores the strategic importance of the cement industry to Tanzania's economic growth and infrastructure development agenda.",
        intro_sw: "Katika uthibitisho muhimu wa ubora wa uzalishaji wa ndani, Waziri Mkuu Kassim Majaliwa alifanya ziara rasmi katika kiwanda cha uzalishaji cha Lake Cement, ambapo alisifia mchango wa kampuni katika maendeleo ya viwanda ya Tanzania na kuhakikisha msaada endelevu wa serikali kwa wazalishaji wa ndani. Ziara hii ya ngazi ya juu inasisitiza umuhimu wa kimkakati wa tasnia ya saruji kwa ukuaji wa uchumi wa Tanzania na ajenda ya maendeleo ya miundombinu.",        paragraphs: [
          "During his comprehensive tour of the facility, Prime Minister Majaliwa inspected the plant's production lines, quality control laboratories, and distribution systems. He expressed particular appreciation for Lake Cement's investment in modern manufacturing technology that has enabled the company to produce cement that meets international quality standards while maintaining competitive pricing for local markets.",
          "The Prime Minister's visit comes as part of the government's broader initiative to promote industrialization through the Tanzania Development Vision 2025, which identifies manufacturing as a key driver of economic transformation. The cement industry occupies a central position in this vision, providing essential materials for the country's ambitious infrastructure development projects.",
          "Lake Cement's facility, which employs over 500 Tanzanians directly and supports thousands more jobs indirectly, showcases the potential of local manufacturing to create employment opportunities and develop technical skills. During his address to company employees, Prime Minister Majaliwa highlighted how such investments contribute to reducing youth unemployment while building Tanzania's technical capacity."
        ],
        paragraphs_sw: [
          "Wakati wa ziara yake ya kina ya kiwanda, Waziri Mkuu Majaliwa alikagua mifumo ya uzalishaji ya kiwanda, maabara za kudhibiti ubora, na mifumo ya usambazaji. Alielezea shukrani za kipekee kwa uwekezaji wa Lake Cement katika teknolojia ya kisasa ya uzalishaji ambayo imewezesha kampuni kuzalisha saruji inayotimiza viwango vya kimataifa vya ubora huku ikidumisha bei za ushindani kwa masoko ya ndani.",
          "Ziara ya Waziri Mkuu inakuja kama sehemu ya mpango mpana wa serikali wa kukuza ujenzi wa viwanda kupitia Dira ya Maendeleo ya Tanzania 2025, ambayo inatambua uzalishaji kama nguzo muhimu ya mabadiliko ya kiuchumi. Tasnia ya saruji inashika nafasi ya kati katika maono haya, ikitoa vifaa muhimu kwa miradi ya maendeleo ya miundombinu yenye malengo makubwa ya nchi.",
          "Kiwanda cha Lake Cement, ambacho kinaajiri Watanzania zaidi ya 500 moja kwa moja na kusaidia maelfu zaidi ya kazi kwa njia isiyo ya moja kwa moja, kinaonyesha uwezekano wa uzalishaji wa ndani kuunda fursa za ajira na kuendeleza ujuzi wa kiufundi. Wakati wa hotuba yake kwa wafanyakazi wa kampuni, Waziri Mkuu Majaliwa alisisitizia jinsi uwekezaji kama huo unavyochangia kupunguza ukosefu wa ajira kwa vijana huku ukijenga uwezo wa kiufundi wa Tanzania."
        ],        quote: {
          text: "What I've witnessed at Lake Cement today exemplifies the type of industrial development Tanzania needs. By investing in modern technology, prioritizing quality, and creating valuable jobs, this company is contributing significantly to our nation's economic transformation. The government remains committed to supporting such investments through appropriate policies and infrastructure.",
          text_sw: "Nilichokiona katika Lake Cement leo ni mfano wa aina ya maendeleo ya viwanda ambayo Tanzania inahitaji. Kwa kuwekeza katika teknolojia ya kisasa, kuweka kipaumbele katika ubora, na kuunda ajira zenye thamani, kampuni hii inachangia kwa kiasi kikubwa mabadiliko ya kiuchumi ya taifa letu. Serikali inabaki kujitolea kusaidia uwekezaji kama huu kupitia sera na miundombinu inayofaa.",
          author: "Hon. Kassim Majaliwa, Prime Minister of Tanzania",
          author_sw: "Mhe. Kassim Majaliwa, Waziri Mkuu wa Tanzania"
        },        additionalContent: [
          "The Prime Minister's assurance of government support includes commitments to address challenges facing local manufacturers, including infrastructure improvements, regulatory streamlining, and measures to protect domestic industry from unfair competition. These policy directions align with Lake Cement's advocacy for a balanced approach that promotes industrial growth while ensuring market competitiveness.",
          "During the visit, Lake Cement's management presented the company's expansion plans, which include increasing production capacity in next years. This growth will enable the company to meet rising demand for construction materials driven by Tanzania's infrastructure development projects and growing real estate sector.",
          "The discussion also covered Lake Cement's sustainability initiatives, including investments in energy-efficient production technology and environmental management systems. Prime Minister Majaliwa commended these efforts, noting their alignment with the government's commitment to environmentally responsible industrial development."
        ],
        additionalContent_sw: [
          "Uhakikisho wa Waziri Mkuu wa msaada wa serikali unajumuisha ahadi za kushughulikia changamoto zinazokabili wazalishaji wa ndani, ikiwa ni pamoja na uboreshaji wa miundombinu, urahisishaji wa udhibiti, na hatua za kulinda tasnia ya ndani dhidi ya ushindani usio wa haki. Mielekeo hii ya sera inaafikiana na utetezi wa Lake Cement kwa mtazamo wa usawa ambao unakuza ukuaji wa viwanda huku ukihakikisha ushindani wa soko.",
          "Wakati wa ziara, uongozi wa Lake Cement uliwasilisha mipango ya upanuzi ya kampuni, ambayo inajumuisha kuongeza uwezo wa uzalishaji katika miaka ijayo. Ukuaji huu utaiwezesha kampuni kukidhi mahitaji yanayoongezeka ya vifaa vya ujenzi yanayotokana na miradi ya maendeleo ya miundombinu ya Tanzania na sekta inayokua ya mali isiyohamishika.",
          "Majadiliano pia yaligusia mipango ya uendelevu ya Lake Cement, ikiwa ni pamoja na uwekezaji katika teknolojia ya uzalishaji yenye ufanisi wa nishati na mifumo ya usimamizi wa mazingira. Waziri Mkuu Majaliwa alisifu juhudi hizi, akitaja kulingana kwake na dhamira ya serikali ya maendeleo ya viwanda yanayozingatia mazingira."
        ],
        conclusion: "Prime Minister Majaliwa's visit to Lake Cement marks an important milestone in the partnership between government and private sector in driving Tanzania's industrial development. The assurance of continued government support reinforces our commitment to expanding production capacity, enhancing quality standards, and contributing to Tanzania's economic transformation through sustainable manufacturing excellence.",
        conclusion_sw: "Ziara ya Waziri Mkuu Majaliwa kwenye Lake Cement ni hatua muhimu katika ushirikiano kati ya serikali na sekta binafsi katika kuongoza maendeleo ya viwanda ya Tanzania. Uhakikisho wa msaada endelevu wa serikali unaimarisha dhamira yetu ya kupanua uwezo wa uzalishaji, kuboresha viwango vya ubora, na kuchangia mabadiliko ya kiuchumi ya Tanzania kupitia ubora wa uzalishaji endelevu."
      },      date: "2021-03-27",
      author: "Corporate Communications",
      author_sw: "Mawasiliano ya Kampuni",
      mainImage: "/images/news/PM-Majaliwa.png",
      gallery: [
        "/images/news/PM-Majaliwa.png",
      ],
      category: "company",
      externalLink: "https://www.thecitizen.co.tz/tanzania/business/majaliwa-assures-investors-of-government-cooperation-2596696",
      relatedArticles: [1, 2, 4]
    },    {        id: 8,        title: "Nyati Cement Engages with contractors at the 2025 ACM organized by Contractors Registration Board",
        title_sw: "Nyati Cement Washiriki na Makandarasi kwenye ACM 2025 iliyoandaliwa na Bodi ya Usajili wa Makandarasi",
        excerpt: "The ACM & Exhibition served as a strategic platform for Nyati Cement to showcase our commitment to quality and innovation in cement manufacturing. The event facilitated direct engagement with contractors and stakeholders in the construction industry.",
        excerpt_sw: "ACM & Maonyesho ilitumika kama jukwaa la kimkakati kwa Nyati Cement kuonyesha dhamira yetu ya ubora na uvumbuzi katika uzalishaji wa saruji. Tukio hilo liliwezesha ushirikiano wa moja kwa moja na makandarasi na wadau katika sekta ya ujenzi.",
        fullContent: {
          intro: "",
          intro_sw: "",
          paragraphs: [
            "The ACM & Exhibition served as a strategic platform for Nyati Cement to showcase our commitment to quality and innovation in cement manufacturing. The event facilitated direct engagement with contractors and stakeholders, enabling us to better understand their needs while gathering valuable insights into market trends and regional distribution challenges.",
            "During the exhibition, footfall at Nyati Cement booth was noticiable due to very informative and engaging booth activities.",
          ],
          paragraphs_sw: [
            "ACM & Maonyesho ilitumika kama jukwaa la kimkakati kwa Nyati Cement kuonyesha dhamira yetu ya ubora na uvumbuzi katika uzalishaji wa saruji. Tukio hilo liliwezesha ushirikiano wa moja kwa moja na makandarasi na wadau, kutuwezesha kuelewa vyema mahitaji yao huku tukikusanya maoni muhimu kuhusu mienendo ya soko na changamoto za usambazaji wa kikanda.",
            "Wakati wa maonyesho, idadi ya wageni katika banda la Nyati Cement ilikuwa kubwa kutokana na shughuli za banda zenye kutoa taarifa na kushirikisha.",
          ],
          quote: {
            text: "",
            author: ""
          },
          additionalContent: [
          ],
         
        },        date: "2025-05-16",
        author: "Corporate Communications",
        author_sw: "Mawasiliano ya Kampuni",
        mainImage: "/images/news/acm/1.jpg",
        gallery: [
          "/images/news/acm/2.jpg",
          "/images/news/acm/3.jpg",
          "/images/news/acm/5.jpg",
          "/images/news/acm/7.jpg",
          "/images/news/acm/8.jpg",
          "/images/news/acm/9.jpg",
          "/images/news/acm/6.jpg"
        ],
        category: "company",
        featured: true,
        tag: "Industry Event",
        relatedArticles: [1, 2, 6]
    },
    {
        id: 7,        title: "Nyati Cement Hands Over Geza Kigamboni Bus Terminal, Citizens Urged to Embrace Opportunities",
        title_sw: "Nyati Cement Yakabidhi Stendi ya Geza Kigamboni, Wananchi Wahimizwa Kutumia Fursa",
        excerpt: "Nyati Cement has officially handed over the newly constructed Kigamboni Bus Terminal to the District Commissioner. The company invested 46 million shillings in this infrastructure project, demonstrating its commitment to supporting President Samia Suluhu Hassan's efforts to bring essential services closer to citizens.",
        excerpt_sw: "Nyati Cement imekabidhi rasmi kituo kipya cha mabasi cha Kigamboni kwa Mkuu wa Wilaya. Kampuni hiyo iliwekeza shilingi milioni 46 katika mradi huu wa miundombinu, ikionyesha dhamira yake ya kusaidia juhudi za Rais Samia Suluhu Hassan za kuleta huduma muhimu karibu na wananchi.",
        fullContent: {
          intro: "In a significant community development initiative, Nyati Cement has officially handed over the newly constructed Kigamboni Bus Terminal to the District Commissioner of Kigamboni, Dar es Salaam on November 8, 2022. This infrastructure project represents another milestone in the company's ongoing commitment to supporting local communities and enhancing public transportation infrastructure.",
          paragraphs: [
            "Speaking during the handover ceremony, Nyati Cement's Chief Plant Operations Officer, Biswajeet Mallik, revealed that the company invested 46 million Tanzanian shillings in the construction of the terminal. He emphasized that this initiative aligns with the company's mission to support President Samia Suluhu Hassan's efforts to bring essential services closer to citizens and improve infrastructure across Tanzania.",
            "Mr. Mallik highlighted that beyond this Geza Bus terminal, Nyati Cement has contributed to multiple sectors including healthcare, education, and infrastructure development in the Kigamboni area. In the healthcare sector, they have supported the Kimbiji Health Center, while their educational contributions include renovating classrooms at Golani School and Kimbiji Secondary School. The company has also provided cement for the construction of the CCM (Chama Cha Mapinduzi) office in Kigamboni.",
            "Mr. Mallik  further noted that Nyati Cement products are being used in major national infrastructure projects, including the Standard Gauge Railway (SGR) construction and the Julius Nyerere Hydropower Project (JNHPP-MW2115) at the Rufiji River basin, showcasing the company's role in Tanzania's most significant development initiatives."
          ],
          quote: {
            text: "Nyati Cement has helped us create this terminal, and we welcome and invite other stakeholders to continue collaborating with us to develop projects for the benefit of our citizens and the people of Kigamboni. We have received this bus terminal with gratitude, and we will continue to give it special attention.",
            author: "Fatma Almas Nyangasa, Kigamboni District Commissioner"
          },
          additionalContent: [
            "The Kigamboni District Commissioner, Fatma Almas Nyangasa, upon receiving the terminal, emphasized that this facility should be viewed as an opportunity for Kigamboni residents to advance both individual and national development. She affirmed that the district government is ready to collaborate with all stakeholders to ensure rapid, positive changes that bring meaningful economic and social development to the people of Kigamboni.",
            "Ms. Nyangasa directed the Kigamboni Municipal Council to work closely with supportive partners and use internal revenue to further improve the terminal area, with the goal of developing it into a major bus terminal for Kigamboni. She expressed confidence that this would become a distinctive terminal for the district and encouraged collaboration to ensure the facility operates efficiently.",
            "Regarding public transportation, the District Commissioner instructed leaders to ensure that buses with permits begin operating from this new terminal. She emphasized that proper regulations would be established to enforce compliance, noting that the area currently being used is not an official terminal space, and that area should be released to its owners for other investments."
          ],
          conclusion: "The handover of the Geza Bus Terminal by Nyati Cement exemplifies how public-private partnerships can effectively contribute to community infrastructure development. As representatives from the Land Transport Regulatory Authority (LATRA) begin issuing permits for the new terminal, this facility is expected to significantly improve public transportation in Kigamboni while creating new economic opportunities for local residents and businesses in the area."
        },
        date: "2022-11-08",
        author: "Corporate Affairs Department",        mainImage: "images/news/Geza Bus Station.webp",        gallery: [
          "/images/news/bs3.webp",
          "/images/news/bs2.webp",
          "/images/news/bs5.webp",
          "/images/news/bs1.webp",
          "/images/news/bs8.png",
          "/images/news/bs4.webp"

        ],
        category: "csr",
        featured: true,
        tag: "Community Development",
        externalLink: "https://www.michuzi.co.tz/2022/11/nyati-cement-wakabidhi-stendi-ya-kigamboni.html",
        relatedArticles: [3, 5, 2]
      },
 {
 id: 9,
title: "Lake Cement Strengthens Community Commitment with Successful Blood Donation Drive",
title_sw: "Lake Cement Yaendeleza Utamaduni wa Kuchangia Damu",
excerpt: "Lake Cement Limited reaffirmed its dedication to social responsibility by hosting a two-day blood donation camp at its Kimbiji plant in partnership with the National Blood Transfusion Service, continuing its eight-year tradition of supporting public health.",
excerpt_sw: "Lake Cement Limited imeendeleza dhamira yake ya uwajibikaji wa kijamii kwa kuandaa kambi ya kuchangia damu ya siku mbili katika kiwanda chake cha Kimbiji kwa kushirikiana na Huduma ya Kitaifa ya Kuhamisha Damu.",
fullContent: {
  intro: "Kigamboni, Tanzania – August 29, 2025 – Lake Cement Limited reaffirmed its dedication to social responsibility by hosting a two-day blood donation camp at its Kimbiji plant on August 28–29, in partnership with the National Blood Transfusion Service (NBTS). The event underscored the company's ongoing commitment to supporting public health and saving lives through regular blood donation initiatives.",
  intro_sw: "Kigamboni, Tanzania – Agosti 29, 2025 – Lake Cement Limited imeendeleza dhamira yake ya uwajibikaji wa kijamii kwa kuandaa kambi ya kuchangia damu ya siku mbili katika kiwanda chake cha Kimbiji mnamo Agosti 28-29, kwa kushirikiana na Huduma ya Kitaifa ya Kuhamisha Damu (NBTS). Tukio hili lilisisitiza dhamira endelevu ya kampuni ya kusaidia afya ya umma na kuokoa maisha kupitia mipango ya kuchangia damu mara kwa mara.",
  paragraphs: [
    "The drive saw enthusiastic participation from employees and local community members, reflecting a shared spirit of generosity and care. An NBTS representative highlighted the critical importance of such efforts, noting the organization's goal to collect 15,000 units of blood annually to meet Tanzania's healthcare needs.",
    "Consistent support from organizations like Lake Cement is essential to maintaining stable blood supplies for emergency and medical treatments across the country's healthcare system.",
    "Beyond its role as a leading cement manufacturer, Lake Cement remains deeply invested in initiatives that promote health, environmental sustainability, and social welfare. This blood donation camp is one of many efforts through which the company fulfills its broader mission of uplifting the communities it serves.",
    "By aligning corporate values with tangible action, Lake Cement continues to set an example of how businesses can play a vital role in addressing societal challenges and supporting national development."
  ],
  paragraphs_sw: [
    "Kampeni hii iliona ushiriki mkubwa wa wafanyakazi na wanajamii wa eneo hilo, kuonyesha roho ya pamoja ya ukarimu na utunzaji. Mwakilishi wa NBTS alisisitiza umuhimu mkubwa wa juhudi hizi, akibainisha lengo la shirika la kukusanya vitengo 15,000 vya damu kila mwaka ili kukidhi mahitaji ya afya ya Tanzania.",
    "Msaada endelevu kutoka mashirika kama Lake Cement ni muhimu katika kudumisha uhifadhi thabiti wa damu kwa dharura na matibabu ya daktari katika mfumo wa afya wa nchi.",
    "Zaidi ya jukumu lake kama mtengenezaji mkuu wa saruji, Lake Cement inaendelea kuwekeza katika mipango inayohamasisha afya, mazingira endelevu, na ustawi wa kijamii. Kambi hii ya kuchangia damu ni mojawapo ya juhudi nyingi ambazo kampuni inazitumia kutimiza lengo lake la kuinua jamii zinazohudumia.",
    "Kwa kuunganisha maadili ya shirika na vitendo vya vitendo, Lake Cement inaendelea kuwa mfano wa jinsi biashara zinavyoweza kuchukua jukumu muhimu katika kushughulikia changamoto za kijamii na kusaidia maendeleo ya kitaifa."
  ],
  quote: {
    text: "Donating blood is a simple yet powerful act of humanity. It costs nothing but means everything to those in need. We are proud to continue our eight-year tradition of hosting blood donation drives and contributing to the well-being of our community.",
    text_sw: "Kuchangia damu ni tendo rahisi lakini lenye nguvu la kibinadamu. Haina gharama yoyote lakini ina maana yote kwa wale wanaohitaji. Tunajivunia kuendeleza utamaduni wetu wa miaka minane wa kuandaa kambi za kuchangia damu na kuchangia ustawi wa jamii yetu.",
    author: "Biswajeet Mallik, Unit Head, Lake Cement",
    author_sw: "Biswajeet Mallik, Mkuu wa Kitengo, Lake Cement"
  },
  additionalContent: [
    "The event reflected a shared spirit of generosity and care among participants, with both employees and community members coming together for this life-saving cause.",
    "Lake Cement's consistent commitment to blood donation over eight years demonstrates how corporate social responsibility can create lasting positive impact in communities."
  ],
  additionalContent_sw: [
    "Tukio hilo lilionyesha roho ya pamoja ya ukarimu na utunzaji miongoni mwa washiriki, wakiwemo wafanyakazi na wanajamii wakikutana kwa lengo hili la kuokoa maisha.",
    "Dhamira endelevu ya Lake Cement ya kuchangia damu kwa kipindi cha miaka minane inaonyesha jinsi uwajibikaji wa kijamii wa mashirika unavyoweza kuunda athari chanya endelevu katika jamii."
  ],
  conclusion: "Through this successful blood donation drive, Lake Cement has once again demonstrated that true corporate leadership includes caring for people and supporting the healthcare system, setting a powerful example of how businesses can contribute meaningfully to societal well-being and national development.",
  conclusion_sw: "Kupitia kampeni hii ya mafanikio ya kuchangia damu, Lake Cement imeonyesha tena kuwa uongozi wa kweli wa shirika unajumuisha kujali watu na kusaidia mfumo wa afya, ukiweka mfano mkuu wa jinsi biashara zinavyoweza kuchangia kwa maana katika ustawi wa kijamii na maendeleo ya kitaifa."
},
  date: "2025-08-30",
  author: "CSR & Communications Department",
  author_sw: "Idara ya CSR na Mawasiliano",
  mainImage: "/images/news/damu2025main.webp",
  gallery: [
    "/images/news/damu20251.webp",
    "/images/news/damu20252.webp",
    "/images/news/damu20253.webp",
    "/images/news/damu20254.webp"
  ],
  category: "csr",
  externalLink: "",
  relatedArticles: [5, 7, 9]
}


  // Additional articles can be added here
];

function getNewsArticle(id) {
  return newsArticles.find(article => article.id.toString() === id.toString());
}

// Function to get article content based on language
function getLocalizedArticleContent(article, language) {
  if (!article) return null;
  
  // Create a localized version of the article
  const localizedArticle = { ...article };
  
  // Apply language-specific content if available
  if (language === 'sw') {
    // Use Swahili versions if available, otherwise fallback to English
    localizedArticle.title = article.title_sw || article.title;
    localizedArticle.excerpt = article.excerpt_sw || article.excerpt;
    localizedArticle.author = article.author_sw || article.author;
    
    // Handle fullContent if it exists
    if (article.fullContent) {
      localizedArticle.fullContent = { ...article.fullContent };
      
      // Handle intro
      if (article.fullContent.intro_sw) {
        localizedArticle.fullContent.intro = article.fullContent.intro_sw;
      }
      
      // Handle paragraphs
      if (article.fullContent.paragraphs_sw) {
        localizedArticle.fullContent.paragraphs = article.fullContent.paragraphs_sw;
      }
      
      // Handle quote if it exists
      if (article.fullContent.quote) {
        localizedArticle.fullContent.quote = { ...article.fullContent.quote };
        
        if (article.fullContent.quote.text_sw) {
          localizedArticle.fullContent.quote.text = article.fullContent.quote.text_sw;
        }
        
        if (article.fullContent.quote.author_sw) {
          localizedArticle.fullContent.quote.author = article.fullContent.quote.author_sw;
        }
      }
      
      // Handle additional content
      if (article.fullContent.additionalContent_sw) {
        localizedArticle.fullContent.additionalContent = article.fullContent.additionalContent_sw;
      }
      
      // Handle conclusion
      if (article.fullContent.conclusion_sw) {
        localizedArticle.fullContent.conclusion = article.fullContent.conclusion_sw;
      }
    }
  }
  
  return localizedArticle;
}

function getRelatedArticles(currentArticleId, count = 3) {
  // First try to get related articles specified by the current article
  const currentArticle = getNewsArticle(currentArticleId);
  if (currentArticle && currentArticle.relatedArticles && currentArticle.relatedArticles.length > 0) {
    // Filter out any related article IDs that don't exist and limit to count
    return currentArticle.relatedArticles
      .map(id => newsArticles.find(article => article.id === id))
      .filter(article => article !== undefined)
      .slice(0, count);
  }
  
  // Fallback: get random articles in the same category, excluding the current one
  const sameCategory = newsArticles
    .filter(article => article.id !== currentArticleId && 
                     article.category === currentArticle.category);
  
  // If there are no articles in the same category, get any other articles
  const otherArticles = newsArticles
    .filter(article => article.id !== currentArticleId);
    // Choose from same category if available, otherwise from other articles
  const pool = sameCategory.length > 0 ? sameCategory : otherArticles;
  
  // Shuffle the pool and pick the first 'count' articles
  return [...pool]
    .sort(() => 0.5 - Math.random())
    .slice(0, count);
}

// Export the news articles
export { newsArticles, getNewsArticle, getRelatedArticles, getLocalizedArticleContent };
